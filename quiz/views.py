        ...
        if is_teacher_authorised(request, code):
            if request.method == 'POST':
                ...
                quiz = Quiz(title=title, description=description, start=start,
                            end=end, publish_status=publish_status, subject=subject)
                quiz.save()
                return redirect('addQuestion', code=code, quiz_id=quiz.id)
            else:
                return render(request, 'quiz/quiz.html', {'subject': subject, 'teacher': Teacher.objects.get(teacher_id=request.session['teacher_id'])})
    ...
    try:
        ...
        if is_teacher_authorised(request, code):
            ...
            if request.method == 'POST':
                ...
                return render(request, 'quiz/addQuestion.html', {'subject': subject, 'quiz': quiz, 'teacher': Teacher.objects.get(teacher_id=request.session['teacher_id'])})
    ...
    if is_teacher_authorised(request, code):
        ...
        for quiz in quizzes:
            ...
            if quiz.start < datetime.datetime.now():
                ...
                quiz.started = False
            quiz.save()
        return render(request, 'quiz/allQuizzes.html', {'subject': subject, 'quizzes': quizzes, 'teacher': Teacher.objects.get(teacher_id=request.session['teacher_id'])})
    else:
        return redirect('std_login')
...
def startQuiz(request, code, quiz_id):
    if is_student_authorised(request, code):
        ...
        return render(request, 'quiz/portalStdNew.html', {'subject': subject, 'quiz': quiz, 'questions': questions, 'total_questions': total_questions, 'student': Student.objects.get(student_id=request.session['student_id'])})
    else:
        return redirect('std_login')
    ...
    if is_student_authorised(request, code):
        ...
        for student_answer in student_answers:
            ...
            quiz.submission_time = student_answer.created_at.strftime(
                "%a, %d-%b-%y at %I:%M %p")
        return render(request, 'quiz/quizResult.html', {'subject': subject, 'quiz': quiz, 'questions': questions, 'student': student})
    else:
        return redirect('std_login')
...
def quizSummary(request, code, quiz_id):
    if is_teacher_authorised(request, code):
        ...
        context = {'subject': subject, 'quiz': quiz, 'questions': questions, 'time': time, 'total_students': total_students,
                   'students': students, 'teacher': Teacher.objects.get(teacher_id=request.session['teacher_id'])}
        return render(request, 'quiz/quizSummaryTeacher.html', context)
